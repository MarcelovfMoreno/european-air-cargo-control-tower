...
* **Target Audience:** Supply Chain Analytics Managers, Logistics Operations Directors, and Aviation Data Engineers.
...
4. **Analytics & Validation (SQL):** Executing analytical queries in SQLite to compute carrier metrics, velocity distributions, turnaround times, and hub density.
5. **Visualization (Tableau):** Building interactive dashboards mapping flight paths, active fleet counts, and speed profiles.

---

## 🔍 SQL Analytics & Query Library (8 Core Queries)

### 1. Active Fleet & Position Volume by Carrier
Counts total logged position updates and distinct active aircraft per carrier.
```sql
SELECT 
    c.carrier_name,
    c.callsign_prefix,
...
2. Fleet Flight Metrics (Avg Speed & Altitude)
Calculates average speed (meters/second) and average altitude (meters) per carrier for active airborne flights (on_ground = 0).
SELECT 
    c.carrier_name,
    f.callsign_prefix,
...
3. Carrier Share (%) of Total Position Observations
Calculates each carrier's telemetry market share relative to total logged position updates using SQL window functions.
SELECT 
    c.carrier_name,
    c.callsign_prefix,
    COUNT(f.icao24) AS total_position_pings,
...
4. Live Flight Operations Log (UTC Timestamps & Status)
Converts raw UNIX timestamps to standard UTC datetime and categorizes aircraft operational status (Airborne vs. On Ground).
SELECT 
    f.callsign,
    f.callsign_prefix,
    c.carrier_name,
    DATETIME(f.time_position, 'unixepoch') AS timestamp_utc,
    f.latitude,
    f.longitude,
    f.baro_altitude AS altitude_m,
    f.velocity AS velocity_mps,
    CASE WHEN f.on_ground = 1 THEN 'On Ground' ELSE 'Airborne' END AS flight_status
FROM fact_flights f
LEFT JOIN dim_carriers c ON f.callsign_prefix = c.callsign_prefix
ORDER BY f.time_position DESC
LIMIT 20;

5. Spatial Proximity to Primary Cargo Hubs (Frankfurt Sector Bounding Box)
Filters flight telemetry falling within the spatial bounding box of Frankfurt Airport (FRA) coordinates.
SELECT 
    f.callsign,
    c.carrier_name,
    DATETIME(f.time_position, 'unixepoch') AS timestamp_utc,
    ...
    f.baro_altitude AS altitude_m,
    f.velocity AS velocity_mps,
    CASE WHEN f.on_ground = 1 THEN 'On Ground' ELSE 'Airborne' END AS status
FROM fact_flights f
LEFT JOIN dim_carriers c ON f.callsign_prefix = c.callsign_prefix
WHERE f.latitude BETWEEN 49.5 AND 50.5
  AND f.longitude BETWEEN 8.0 AND 9.0
ORDER BY f.time_position DESC;

6. Frankfurt Hub Operations Summary & Operational Distribution
Aggregates tracked traffic around Frankfurt (FRA) by carrier, separating ground versus airborne telemetry pings.
SELECT 
    c.carrier_name,
    c.callsign_prefix,
    COUNT(DISTINCT f.callsign) AS unique_flights_tracked,
    COUNT(f.icao24) AS total_hub_pings,
    SUM(CASE WHEN f.on_ground = 1 THEN 1 ELSE 0 END) AS ground_pings,
    SUM(CASE WHEN f.on_ground = 0 THEN 1 ELSE 0 END) AS airborne_pings
FROM fact_flights f
JOIN dim_carriers c ON f.callsign_prefix = c.callsign_prefix
WHERE f.latitude BETWEEN 49.5 AND 50.5
  AND f.longitude BETWEEN 8.0 AND 9.0
GROUP BY c.carrier_name, c.callsign_prefix
ORDER BY total_hub_pings DESC;

7. Cross-Hub Operations Comparison (FRA vs. LEJ vs. LGG)
Categorizes and compares simultaneous operational volume across Central Europe's top three air cargo hubs.
SELECT 
    CASE 
...
SELECT 
    f.callsign,
    c.carrier_name,
    MIN(DATETIME(f.time_position, 'unixepoch')) AS first_seen_utc,
    MAX(DATETIME(f.time_position, 'unixepoch')) AS last_seen_utc,
    COUNT(f.icao24) AS ground_pings_count
FROM fact_flights f
JOIN dim_carriers c ON f.callsign_prefix = c.callsign_prefix
WHERE f.on_ground = 1
